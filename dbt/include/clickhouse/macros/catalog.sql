{% macro clickhouse__get_catalog(information_schema, schemas) -%}
  {%- call statement('catalog', fetch_result=True) -%}
    select
      ...
      if(tables.engine not in ('MaterializedView', 'View'), 'stream', 'view') as table_type,
      null_if(tables.comment, '') as table_comment,
      columns.name as column_name,
      columns.position as column_index,
      columns.type as column_type,
      null_if(columns.comment, '') as column_comment,
      null as table_owner
