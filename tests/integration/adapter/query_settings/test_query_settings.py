...
class TestNullableColumnJoin:
    ...
    def test_nullable_column_join(self, project):
        ...
        result = project.run_sql(
            "select is_nullable(test_id) as is_nullable_column from nullable_column_model",
            fetch="one",
        )
        assert result[0] == 1
...
class TestNotNullableColumnJoin:
    ...
    def test_nullable_column_join(self, project):
        ...
        result = project.run_sql(
            "select is_nullable(test_id) as is_nullable_column from not_nullable_column_model",
            fetch="one",
        )
        assert result[0] == 0
